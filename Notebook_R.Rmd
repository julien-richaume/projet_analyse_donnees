...
fig_cluster_pca <- plot_ly(data = df_clusters, x = ~longitude, y = ~latitude, type = "scattermapbox",
                           color = ~cluster, colors = c("red", "blue","pink"),
...
fig_alt <- plot_ly(data = df_bonus, x = ~longitude, y = ~latitude, type = "scattermapbox",
                   color = ~bonus, colors = c("goldenrod", "pink"),
...
df_clusters_pca <- data.frame(
  latitude = Position[,2],
  longitude = Position[,1],
  cluster = as.integer(clusters_ac_raw_sorted)
)
...
ggplot(df_gmm, aes(x=pc1, y=pc2, col=cluster))+
  geom_point() + 
...
for (i in 1:k_max){
  gmm_model <- Mclust(velib_PCA_reduced, G = i)
  bic = rbind(bic, c(i, gmm_model$bic))
}
bic <- bic %>% rename(
      K = X1, 
      BIC = X.33292.0596572221)
ggplot(bic, aes(x=K, y=BIC)) +
  geom_point()
```

```{r}
velib_PCA_scaled <- scale(velib_PCA_reduced)
```

```{r}
silhouette_score = data.frame()

for (i in 1:k_max){
  gmm_predicted <- predict(Mclust(velib_PCA_reduced, G = i))
  silhouette_score = rbind(silhouette_score, c(i, silhouette(gmm_predicted$classification, velib_PCA_scaled)))
}

head(silhouette_score)

silhouette_score <- silhouette_score %>% rename(
      K = X1L, 
      Silhouette = X.33292.0596572221)
ggplot(silhouette_score, aes(x=K, y=Silhouette)) +
  geom_point()

```

```{r}
# Define the draw_ellipse function
draw_ellipse <- function(mean, covariance, alpha, col = "#CCCCCC") {
  eig <- eigen(covariance)
  U <- eig$vectors
  s <- sqrt(eig$values)
  angle <- atan2(U[2, 1], U[1, 1]) * (180 / pi)
  width <- 2 * s[1]
  height <- 2 * s[2]
  
  t <- seq(0, 2 * pi, length.out = 100)
  x <- mean[1] + width * cos(t) * cos(angle) - height * sin(t) * sin(angle)
  y <- mean[2] + width * cos(t) * sin(angle) + height * sin(t) * cos(angle)
  
  lines(x, y, col = col, lwd = 2)
  polygon(x, y, col = adjustcolor(col, alpha), border = NA)
}

plotGMM <- function(gmm, data, n_clusters = 6) {
  gmm$cl <- predict(gmm, data)$classification
  clusters_gmm <- gmm$cl
  
  plot(data[,1], data[,2], col = clusters_gmm + 1, pch = 20, cex = 1.5)
  
  for (i in 1:n_clusters) {
    mean <- gmm$parameters$mean[, i][1:2]
    covariance <- gmm$parameters$variance$sigma[, , i][1:2,1:2]
    weight <- gmm$parameters$pro[i]
    draw_ellipse(mean, covariance, weight, col = palette[i])
  }
}
...
gmm <- Mclust(velib_PCA_reduced, G = K)
