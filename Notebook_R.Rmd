...
for (i in 0:3) {
  for (j in 0:3) {
    index <- 4 * i + j + 1
    plot(Time, loading_transposed[, index], type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "Loading",
         main = Coord$names[index])
    abline(v = seq(1, p, length.out = 8), col = "black", lty = "dotted")
  }
}
...
data23h_df <- Coord
data23h_df$vingt_troisheures=data23h
fig23 <- plot_ly(data = data23h_df, x = ~longitude, y = ~latitude, type = "scattermapbox",
                    color = ~vingt_troisheures, colors = c("blue", "yellow"),
...
#On calculer la moyenne des données de chargement pour chaque ligne
