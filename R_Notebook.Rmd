...
palette = c("#8dd3c7", "#bebada", "#db564d", "#64ade8", "#fdb462", "#b3de69", "#fccde5", "#d9d9d9", "#bc80bd")
...
On garde aussi 10 classes (en regardant le max du BIC)


#Projection des 10 clusters sur les 2 premières composantes principales
...
palette = c("#8dd3c7", "#ffffb3", "#bebada", "#db564d", "#64ade8", "#fdb462", "#b3de69", "#fccde5", "#d9d9d9", "#bc80bd")
...
On observe pour chaque cluster le boxplot des probas d'appartenance de chaque individu. Par exemple le cluster est le mieux défini : la proba moyenne des individus est proche de 1 et la variance est faible, les individus sont donc tous quasiment certains d'appartenir à ce cluster, il y a peu de mal classés dans le n°3.

Au contraire, le cluster 5 a une grande variance est une moyenne de proba d'appartenance égale à 0.75, les individus sont donc moins certains d'appartenir à ce cluster.
...
Avec le critère ICL, on garde également le modèle VVE mais avec seulement 7 clusters.
...
#Projection des 7 clusters sur les 2 premières composantes principales
...
Ici on voit que les clusters 2,5 et 7 sont ceux où les individus ont une plus grande probabilité d'appartenance. On remarque en général que tous les clusters sont bien définis car les moyennes sont hautes.

## Projection des 7 clusters sur la carte de Paris
...
grid.arrange(p1, p2, ncol = 2)
```
Ici on voit que le cluster n°2 est celui où les individus ont une plus grande probabilité d'appartenance. On remarque en général que tous les clusters sont bien définis car les moyennes sont hautes (on a par contre des variances plus grandes pour les autres clusters que le 2)
...
```
L'index ajusté vaut 0.26 ce qui montre que les deux clusterings ne correspondent pas beaucoup.
