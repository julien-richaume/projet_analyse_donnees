...
for (i in 2:8){
  ...
  if (i*24!=len_X){
    ...
    print(0)
    i_fin_nuit = 7
    nuit_dimanche = group_by(loading[i_debut_nuit:i_debut_nuit+1],loading[1:i_fin_nuit])
    print(nuit_dimanche)
    new_Col = rowMeans(nuit_dimanche)
  }
  breaks = c(-0.1,0.33,0.66, 1.01)
  newCol = cut(new_Col, breaks = breaks, labels = load)
  colname = paste("Nuit", i-1, sep="")
  X_quali[[colname]] = newCol
}
...
#Il faut garder 7 composantes pour garder 70% de la variance, 11 pour 80%
...
# dim 5
fviz_contrib(res.mca, choice="var", axes=5, top=20)
# dim 6 ??????
fviz_contrib(res.mca, choice="var", axes=6, top=20)
...
```{r}
# column cos2 similarities
res.ca$col$cos2

# row cos2 similarities
res.ca$row$cos2
```
